...
var (
	// ErrUserAlreadyExists is returned when attempting to register an email address that is already associated with an existing user.
	ErrUserAlreadyExists = errors.New("emailpassword: user already exists")

	// ErrUserNotFound is returned by repository methods when no user matches the given identifier or email.
	ErrUserNotFound = errors.New("emailpassword: user not found")

	// ErrAccountNotFound is returned when an authentication credentials record is not found for the specified user and provider.
