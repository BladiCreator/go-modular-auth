...
func DefaultNumericOTPGenerator(length int) (string, error) {
	...
	for i := 0; i < length; i++ {
		...
		if err != nil {
			return "", fmt.Errorf("emailotp: failed to generate secure random number: %w", err)
		}
...
func NewAESGCMCipher(secretKey string) (*AESGCMCipher, error) {
	if secretKey == "" {
		return nil, errors.New("emailotp: secret key cannot be empty for encrypted store mode")
	}
...
func (c *AESGCMCipher) Decrypt(encrypted string) (string, error) {
	...
	if len(data) < gcm.NonceSize() {
		return "", errors.New("emailotp: malformed ciphertext")
	}
