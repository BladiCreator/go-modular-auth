...
// verifyStoredOTP verifies the provided OTP against the stored value in constant time.
...
func (p *Plugin) resolveOTP(ctx context.Context, identifier string, email string, otpType OTPType) (string, time.Time, error) {
	...
	if p.config.GenerateOTP != nil {
		otp, err = p.config.GenerateOTP(ctx, email, otpType)
	} else {
		otp, err = DefaultNumericOTPGenerator(p.config.OTPLength)
	}
	if err != nil {
		return "", time.Time{}, err
	}

	storedValue, err := p.storeOTP(otp)
	if err != nil {
		return "", time.Time{}, err
	}
...
func (p *Plugin) atomicVerifyOTP(ctx context.Context, identifier, providedOTP string, otpType OTPType, email string, extra map[string]any) error {
	...
	if err == nil && existing != nil && existing.ExpiresAt.Before(time.Now()) {
		_ = p.repo.DeleteVerificationValue(ctx, identifier)
		p.publishEvent(EventEmailOTPExpired, &OTPFailedPayload{
			Email:  email,
			Type:   otpType,
			Reason: "expired",
			Extra:  extra,
	...
	if err != nil || consumed == nil {
		p.publishEvent(EventEmailOTPFailed, &OTPFailedPayload{
			Email:  email,
			Type:   otpType,
	...
	if attempts >= p.config.AllowedAttempts {
		p.publishEvent(EventEmailOTPAttemptsExceeded, &OTPFailedPayload{
			Email:        email,
			Type:         otpType,
